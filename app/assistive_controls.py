...
from .audio_utils import AudioLevelMonitor, capture_voice_clip_until_silence, prepare_audio_for_asr, save_audio_clip
...
from .voice_router import normalize_text, resolve_command_entry
...
VOICE_COMMAND_CONFIDENCE_THRESHOLD = 0.68
QUIT_CONFIRMATION_WINDOW_S = 12.0
QUIT_CONFIRMATION_PHRASES = {"confirmar", "confirmo", "si", "si confirmo", "si cerrar"}
...
class AssistiveController:
    def __init__(self, profile: dict, settings: dict, main_gui_interface=None):
        ...
        # Failsafe activo: llevar el mouse físico a la esquina superior
        # izquierda aborta la acción en curso y pausa el sistema (mecanismo de
        # escape para el cuidador si el cursor se descontrola).
        ...
        self.voice_max_record_seconds = float(settings.get("voice_max_record_seconds", 6.0))
        self.voice_min_record_seconds = float(settings.get("voice_min_record_seconds", 1.0))
        self.voice_silence_seconds = float(settings.get("voice_silence_seconds", 0.8))
        self.voice_debug_save_clip = bool(settings.get("voice_debug_save_clip", False))
        self.voice_model_size = str(settings.get("voice_model_size", "small"))
        self.tts = TtsWorker() if self.tts_enabled else None
        # El modelo Whisper se carga en background en start() para no congelar
        # el arranque de la aplicación.
        self.whisper: WhisperModel | None = None
        self.whisper_state = "sin cargar" if self.voice_enabled else "deshabilitado"
        ...
        self._pending_quit_deadline = 0.0

        heuristic_thresholds = profile.get("heuristic_thresholds", {})
        self.hybrid_eye_closed_confidence_floor = float(
            heuristic_thresholds.get("hybrid_eye_closed_confidence_floor", 0.18)
        )
        self.hybrid_eye_closed_ratio_max = float(heuristic_thresholds.get("hybrid_eye_closed_ratio_max", 0.19))
        self.hybrid_eye_closed_asymmetry_max = float(
            heuristic_thresholds.get("hybrid_eye_closed_asymmetry_max", 0.08)
        )
        self.heuristic_blink_closed_ratio = float(heuristic_thresholds.get("blink_closed_ratio", 0.17))
        self.heuristic_blink_open_ratio = float(heuristic_thresholds.get("blink_open_ratio", 0.20))
        self.heuristic_blink_asymmetry_min = float(heuristic_thresholds.get("blink_asymmetry_min", 0.04))
        self.heuristic_blink_confidence = float(heuristic_thresholds.get("blink_confidence", 0.72))
    ...
    def start(self):
        ...
        if self.voice_enabled and self.whisper is None:
            threading.Thread(target=self._load_whisper, daemon=True).start()
        if self.model_load_error:
            self._publish_event(f"Modelo de gestos no cargado: {self.model_load_error}")
            self.logger.log("error", "Modelo de gestos incompatible", error=self.model_load_error)
        self.logger.log(
            "session",
            "Inicio de sesión",
    ...
    def _load_whisper(self):
        self.whisper_state = "cargando"
        self._publish_event(f"Cargando motor de voz ({self.voice_model_size})...")
        try:
            model = WhisperModel(self.voice_model_size, device="cpu", compute_type="int8")
        except Exception as exc:
            self.whisper_state = "error"
            self.logger.log("error", "No se pudo cargar el motor de voz", error=str(exc))
            self._publish_event(f"Error cargando motor de voz: {exc}")
            return
        self.whisper = model
        self.whisper_state = "listo"
        self._publish_event("Motor de voz listo.")
    ...
    def toggle_cursor_control(self):
        ...
        self.logger.log("cursor", "Cambio de clutch", cursor_active=cursor_active)
        self.hablar("Cursor activado." if cursor_active else "Cursor congelado.")
    ...
    def set_cursor_control(self, enabled: bool):
        ...
        self.logger.log("cursor", "Cursor configurado", cursor_active=enabled)
        self.hablar("Cursor activado." if enabled else "Cursor congelado.")
    ...
    def toggle_pause(self):
        ...
        self.logger.log("state", "Cambio de pausa", state=new_state.value)
        self.hablar("Sistema reanudado." if new_state == AppState.READY else "Sistema en pausa.")
    ...
    def _pause_for_failsafe(self):
        with self._state_lock:
            if self.state != AppState.READY:
                return
            self.state = AppState.PAUSED
        self.logger.log("state", "Pausa por failsafe (esquina de pantalla)")
        self.hablar("Sistema pausado por seguridad. Di reanudar sistema para continuar.")
    ...
    def _main_loop(self):
        ...
        while self.running:
            ...
            frame_rgb = self._render_overlay(
                snapshot.frame_bgr, snapshot.face_sample, control_state, prediction, compact=compact
            )
            if compact:
                self._push_gui_frame(None, frame_rgb)
            else:
                self._push_gui_frame(frame_rgb, None)
    ...
    def _start_voice_listener(self, trigger_source: str = "gesture"):
        ...
        if self.whisper is None:
            if self.whisper_state == "cargando":
                self._publish_event("El motor de voz aún se está cargando, intenta en unos segundos.")
            else:
                self._publish_event("Motor de voz no disponible.")
            return
    ...
    def _dispatch_voice_command(
        ...
        """Ejecuta el comando de voz aplicando la confirmación de cierre.

        Devuelve (handled, status_text). handled=False → comando no reconocido.
        """
        now = time.monotonic()
        pending_quit = now < self._pending_quit_deadline
        recognized = command_entry is not None and confidence >= VOICE_COMMAND_CONFIDENCE_THRESHOLD
        command_id = command_entry["id"] if command_entry else None

        if pending_quit and (
            (recognized and command_id == "quit")
            or normalize_text(transcript) in QUIT_CONFIRMATION_PHRASES
        ):
            self._pending_quit_deadline = 0.0
            self._publish_event("Cierre confirmado por voz.")
            self.action_service.execute_builtin_voice_command("quit", previous_state.value)
            return True, "Cerrando el sistema."

        if not recognized:
            return False, ""

        if command_id == "quit":
            self._pending_quit_deadline = now + QUIT_CONFIRMATION_WINDOW_S
            self.hablar("Para cerrar el sistema, repite cerrar sistema o di confirmar.")
            return True, "Esperando confirmación para cerrar el sistema."

        if pending_quit:
            self._pending_quit_deadline = 0.0
            self._publish_event("Cierre cancelado por otro comando.")
        ...
        return True, f"Comando de voz: {command_entry['label']}"
    ...
    def _listen_once(self, trigger_source: str = "gesture"):
        ...
        self.logger.log(
            ...
            max_duration_s=self.voice_max_record_seconds,
            sample_rate=self.voice_sample_rate,
        )
        try:
            if self.audio_level_monitor is not None:
                self.audio_level_monitor.suspend()
            recording, audio_diag = capture_voice_clip_until_silence(
                self.voice_sample_rate,
                device=self.audio_input_device,
                max_duration_s=self.voice_max_record_seconds,
                min_duration_s=self.voice_min_record_seconds,
                silence_duration_s=self.voice_silence_seconds,
            )
            if self.voice_debug_save_clip:
            ...
            segments, _ = self.whisper.transcribe(
                prepare_audio_for_asr(recording),
                language="es",
                vad_filter=True,
            )
            ...
            handled, dispatch_status = self._dispatch_voice_command(
                command_entry, confidence, text, previous_state
            )
            if handled:
                self.last_voice_state = "comando reconocido"
                command_label = command_entry["label"] if command_entry else "Cerrar sistema"
                self._publish_event(f"Comando reconocido: {command_label} ({confidence:.2f})")
                self._push_status(
                    dispatch_status,
                    command_label,
                    confidence,
    ...
    def _render_overlay(self, frame_bgr, sample, control_state, prediction, compact: bool = False):
    ...
    def _push_status(
        ...
        *,
        ...
        payload = {
            ...
            "voice_engine": self.whisper_state,
            "state": self.state.value,
